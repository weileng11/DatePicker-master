...
public class MonthView extends View {
    ...
    private int centerYear, centerMonth,centerDay;
    ...
    private void draw(Canvas canvas, int x, int y, int year, int month) {
        ...
        DPInfo[][] info = mCManager.obtainDPInfo(year, month,centerDay);
        DPInfo[][] result;
        Region[][] tmp;
    ...
    private void defineRegion(int x, int y) {
        DPInfo[][] info = mCManager.obtainDPInfo(centerYear, centerMonth,centerDay);
        Region[][] tmp;
        if (TextUtils.isEmpty(info[4][0].strG)) {
            tmp = MONTH_REGIONS_4;
        } else if (TextUtils.isEmpty(info[5][0].strG)) {
        ...
        for (int i = 0; i < tmp.length; i++) {
            for (int j = 0; j < tmp[i].length; j++) {
                ...
                if (TextUtils.isEmpty(mCManager.obtainDPInfo(centerYear, centerMonth,centerDay)[i][j].strG)) {
                    continue;
                }
                if (region.contains(x, y)) {
                    ...
                    if (mDPMode == DPMode.SINGLE) {
                        ...
                        final String date = centerYear + "-" + centerMonth + "-" +
                                mCManager.obtainDPInfo(centerYear, centerMonth,centerDay)[i][j].strG;
